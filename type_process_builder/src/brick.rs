...
impl<'same_process, SERIALIZER: Serializer, PARAM_VALUE: ParamValue, TAIL: ParamList>
    ParamList<'same_process, SERIALIZER> for HCons<PARAM_VALUE, TAIL>
{
    fn get_params(self) -> Vec<Value> {
...
impl<
        ...
        TAIL: ParamReprList<'same_process, SERIALIZER>,
...
>
{
    async fn handle(
        &self,
        input: CONSUMES::VALUE,
    ) -> anyhow::Result<(Option<Message>, PRODUCES::VALUE)>;
