...
use egui::{Context, Key, Label, Order, PointerButton, Response, Stroke, TextEdit, Ui, Vec2, Widget};
use egui::Key::M;
use crate::brick::*;
...
impl TemplateApp {
  ...
  pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
    ...
    if let Some(storage) = cc.storage {
      ...
      app.next_brick_uuid = app.bricks.iter().map(|brick| brick.uuid).max_by(|left, right| left.cmp(&right)).unwrap_or(0) + 1;
      return app;
    }
  ...
  fn edges(&mut self, ui: &Ui) {
    ui.painter().extend(self.edges.iter().filter_map(|edge| {
      if let (Some(a), Some(b)) = (self.bricks.iter().find(|brick| brick.uuid == edge.0), self.bricks.iter().find(|brick| brick.uuid == edge.1)) {
  ...
  fn bricks(&mut self, ctx: &Context, ui: &mut Ui) {
    if let (true, Some(mouse), Some(edge_start)) = (ctx.input(|i| { i.pointer.primary_released() }), ctx.pointer_latest_pos(), self.edge_start) {
  ...
  fn retain_bricks_with_add_edge(&mut self, ui: &mut Ui, create_edge: impl Fn((&BrickModel, Rect)) -> Option<(usize, usize)>) -> Option<(usize, usize)> {
    ...
    self.bricks.retain_mut(|brick_model| {
      ...
      if keep.not() {
        self.edges.retain(|edge| edge.0 != brick_model.uuid && edge.1 != brick_model.uuid)
      }
      keep
    }
    );
  ...
  fn create_edge(mouse: Pos2, edge_start: usize, brick_model: &BrickModel, brick_rect: Rect) -> Option<(usize, usize)> {
