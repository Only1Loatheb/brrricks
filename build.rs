...
pub fn main() {
  monk::init(); // causes this file to rerun every push
...
fn update_diagram_in_readme(diagram_path: &Path, section_header: &str) {
  ...
  let start =
    readme.find(section_header).unwrap_or_else(|| panic!("`{section_header}` section not found in {readme_path:?}"));

  // Position just after the section header line
  let content_start = readme[start..].find('\n').map(|i| start + i + 1).expect("Malformed section header");
